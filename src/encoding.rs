...
pub trait Encoding {
    unsafe fn encode(values: *mut u8, count: &mut usize) -> *mut u8;
    unsafe fn decode(values: *mut u8, count: &mut usize) -> *mut u8;
}
    ...
    E: Encoding,
{
    ...
    pub fn encode(&mut self) {
        ...
        unsafe {
            ...
            let new_data = E::encode(self.array.get_raw_ptr(), &mut new_count);
    ...
    pub fn decode(&mut self) {
        ...
        unsafe {
            let mut new_count = self.get_count();
            let new_data = E::decode(self.array.get_raw_ptr(), &mut new_count);
            self.array.set_backing_data(new_data, new_count, new_count);
            self.encoded = false;
        }
    }
